...
### 浏览器安全上下文与哈希实现踩坑（HTTPS vs HTTP）

#### 问题经过

- 最初前端计算 `file_hash` 时，**一律使用浏览器的 `crypto.subtle.digest('SHA-256', ...)`** 计算文件内容哈希。
- 在 **HTTPS / localhost** 场景，这个实现是正常的；但在某些 **非安全上下文（如 http + 公网 IP）** 下，浏览器不开放 `crypto.subtle` 能力：
  - `crypto` 对象存在但 `crypto.subtle` 为 `undefined`，或其 `digest` 方法不可用；
  - 直接调用会抛出异常，前端把“哈希计算失败”视为上传失败，导致**用户一点击上传就报错**。
- 现象：同一份前端代码，在开发环境（localhost / HTTPS）一切正常，切到某些 HTTP 环境后，**还没真正开始上传到 OSS，前端就因为 hash 失败直接宣告上传失败**。

#### 最终方案：按浏览器能力分支，安全上下文用 SHA-256，非安全上下文降级

- 在 `dragons-frontend` 的 `computeFileHash(file)` 中，改为**先探测浏览器是否支持安全哈希能力**：
  - 若 `typeof crypto !== 'undefined' && crypto.subtle && typeof crypto.subtle.digest === 'function'`：
    - 认为处于安全上下文（HTTPS / localhost 等），**使用 SHA-256 计算真实内容哈希**，保证幂等性和唯一性；
  - 否则：
    - 认为处于非安全上下文或受限环境，**不再强行调用 SHA-256**，而是退化为“基于文件元数据的伪哈希”：
    - 具体形式类似：`fallback:${name}|${size}|${lastModified}|${type}`。
- 这样：
  - **HTTPS/localhost 场景**：继续享受真正的内容哈希，保证去重精度；
  - **HTTP / 受限场景**：尽量用“文件名 + 大小 + 修改时间 + 类型”组合出一个相对稳定的标识，不再因为 `crypto.subtle` 缺失而中断上传流程。

#### 降级方案的影响与取舍

- **风险**：伪哈希不再保证内容级别的唯一性，极端情况下可能出现不同文件生成相同 `fallback` 值。
- **当前接受的取舍**：
  - 后端依然把 `file_hash` 当作“**幂等的强提示**”，而不是“绝对不会碰撞的真值”；
  - 实际场景中，“完全相同的名称 + 大小 + 修改时间 + 类型”在不同文件上的碰撞概率较低，可以接受；
  - 更重要的是：**避免了在 HTTP 场景下用户完全无法上传的致命问题**。
- 经验教训：
  - 浏览器端依赖 `crypto.subtle`、`Clipboard` 等能力时，要明确它们与 **安全上下文（HTTPS）强相关**，在设计时就考虑能力探测与降级路径；
  - 上传链路里，一旦“前置校验/预处理”失败（如 hash 计算），要确保**能给用户清晰错误提示**，同时尽量提供可用的退化方案，而不是简单中断整个流程。
