...
## 2026-02-12（前端批量上传图片：保持后端单文件接口）

### 背景与目标
- 后端上传接口 `POST /api/media/upload` 当前一次只接收一个文件。
- 为了提升效率，在前端实现“选择文件夹/多选图片 → 生成队列 → 逐个调用后端上传接口”的批量上传能力。

### 实现要点
- **浏览器安全限制**：前端不能直接遍历本地目录，只能通过文件选择器授权获取 `FileList`：
  - `webkitdirectory` 选择文件夹（Chromium 系浏览器支持）
  - `multiple` 多选图片兜底（兼容性更好）
- **批量策略**：
  - 元信息（标题/简介/成员标签）只填一次，应用到每一张图片
  - 单张失败不影响后续（继续上传，记录失败原因）
- **错误处理**：后端可能返回 HTTP 200 但 `code != 200`（业务失败），前端按 `code` 判断并显示 `message`。

### 前端改动（高层）
- `dragons-frontend/src/api/media.js`：新增 `uploadMedia(formData)`，复用 axios 实例与 JWT 头注入。
- `dragons-frontend/src/views/UploadMedia.vue`：新增上传方式切换（单张/批量），批量模式维护队列并顺序上传。
- `dragons-frontend/src/assets/styles/media-browse.css`：补齐“上传方式切换/批量选择/队列列表/状态徽章”样式。

### 迭代记录（UI 细化）
- 将“图片/视频”入口彻底拆分为两个按钮：**上传图片 / 上传视频**，不再手动选择类型，类型由入口按钮决定。
- 图片上传统一使用“队列窗口”呈现：队列里只有 1 张图片也视为单张上传；“选择多张图片”改为“添加图片”。
- 视频上传的“选择封面”按钮移动到封面图右下角（浮层按钮），并将文案从“上传封面”调整为“选择封面”。
- 上传页面整体容器宽度从 600px 调整为 920px，使队列列表与表单信息更舒展。
- 优化上传页面的“模式切换”体验：切换“上传图片/上传视频”时不再清空已选择的图片队列与表单内容；仅在离开页面（返回/取消）或刷新时自然清空。

### 反思与改进
- 批量上传的前端队列模式适合作为 MVP；若未来文件量很大或需要更强的并发/断点续传能力，可考虑新增后端批量接口或引入专门的大文件上传方案（分片/断点续传）。
