      ...
      <div v-if="visible" class="modal-overlay" @click.self="close">
        <div class="modal-box auth-modal">
          ...
          <form class="auth-form" @submit.prevent="handleSubmit">
            ...
            <button type="submit" class="btn-submit" :disabled="loading">确认找回</button>
          </form>
        </div>
      </div>
    </Transition>
...
watch(() => props.visible, (v) => {
  if (v) {
    form.value = { loginName: '', phoneNumber: '', newPassword: '', confirmPassword: '' }
    errorMsg.value = ''
    successMsg.value = ''
    showNew.value = false
...
async function handleSubmit() {
  ...
  try {
    ...
    if (res && res.code !== 200) {
      errorMsg.value = res.message || '找回失败，请检查登录名与手机号后重试'
      return
    }
    ...
    errorMsg.value = e?.response?.data?.message || e?.message || '找回失败，请检查登录名与手机号后重试'
  } finally {
    loading.value = false
  }
}
